...
export function BrandKit({ site, onBanner }) {
  ...
  return html`<${Card}><div class="p-4">
    ...
    ${open && html`<div class="mt-3 pt-3 border-t border-slate-100 space-y-3">
      ...
      <${Field} label="🔧 Visual truth rules (optional — trade facts AI images must NEVER get wrong: where controls and hardware really live, what equipment looks like, what should never appear). Lasting preferences from image feedback (yours or the client's) are ALSO distilled into a '— Learned from client feedback —' section below your rules automatically — review, edit, or delete those lines any time."><${Textarea} value=${f.visualTruth} onInput=${(v) => setF({ ...f, visualTruth: v })} rows=${3} placeholder="The fireplace damper handle is INSIDE the firebox, never on a wall. Dryer vents exit through the roof or exterior wall, never the attic. Our techs always wear harnesses on roofs…" /></${Field}>
      <div class="text-[11px] text-slate-400">AI image models don't know the trades — these rules ride along on every image generation so hardware is never drawn in the wrong place. After adding one, use “Regenerate all images” on an affected month.</div>
      <div>
