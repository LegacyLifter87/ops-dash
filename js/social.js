...
export function BrandKit({ site, onBanner }) {
  ...
  return html`<${Card}><div class="p-4">
    ...
    ${open && html`<div class="mt-3 pt-3 border-t border-slate-100 space-y-3">
      ...
      <${Field} label="🔧 Visual truth rules (optional — trade facts AI images must NEVER get wrong: where controls and hardware really live, what equipment looks like, what should never appear)"><${Textarea} value=${f.visualTruth} onInput=${(v) => setF({ ...f, visualTruth: v })} rows=${3} placeholder="The fireplace damper handle is INSIDE the firebox, never on a wall. Dryer vents exit through the roof or exterior wall, never the attic. Our techs always wear harnesses on roofs…" /></${Field}>
